...
def ex8(conn):
    # FIXED: Cast OrderDate to DATE before extracting
    sql_statement = """
    SELECT 
        CAST((EXTRACT(MONTH FROM "o"."OrderDate"::DATE) + 2) / 3 AS INTEGER) as "Quarter",
        CAST(EXTRACT(YEAR FROM "o"."OrderDate"::DATE) AS INTEGER) as "Year",
        "c"."CustomerID",
...
def ex11(conn):
    # FIXED: Cast OrderDate to DATE for calculation
    sql_statement = """
    ...
    SELECT 
        ...
        ("ld"."OrderDate" - "ld"."PreviousOrderDate") as "MaxDaysWithoutOrder"
    FROM LaggedDates "ld"
...
def ex9(conn):
    ...
    WITH CustomerSales AS (
        SELECT 
            'Q' || CAST(EXTRACT(QUARTER FROM "o"."OrderDate"::DATE) AS TEXT) as "Quarter",
            CAST(EXTRACT(YEAR FROM "o"."OrderDate"::DATE) AS INTEGER) as "Year",
            "c"."CustomerID",
            ROUND(CAST(SUM("p"."ProductUnitPrice" * "o"."QuantityOrdered") AS NUMERIC), 2) as "Total"
        FROM "orderdetail" "o"
